...
VehicleSchema.methods.calculateHealthScore = function() {
    ...
    const maintenanceFactor = this.maintenanceHistory.length > 0 ? 100 : 80;
...
VehicleSchema.methods.predictNextMaintenance = function() {
    const lastMaintenanceMileage = this.maintenanceHistory.length > 0 ? 
        Math.max(...this.maintenanceHistory.map(m => m.mileage)) : 0;
    
    this.nextMaintenanceMileage = lastMaintenanceMileage + this.maintenanceInterval;
    
    // Predict date based on average daily mileage
    const daysSinceLastMaintenance = this.maintenanceHistory.length > 0 ? 
    ...
    const daysUntilNextMaintenance = (this.nextMaintenanceMileage - this.currentMileage) / averageDailyMileage;
